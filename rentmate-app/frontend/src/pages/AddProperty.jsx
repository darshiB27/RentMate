...
// Purpose: Multi-field form for listing creations with inputs validation schemas and file uploads.
...
import { createPropertyRequest } from '../features/properties/services/propertyApi.js';
...
export default function AddProperty() {
  ...
  const {
    ...
    defaultValues: {
      amenities: [],
      latitude: 12.9716, // Bangalore default center coordinates
      longitude: 77.5946,
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: (formData) => createPropertyRequest(formData),
    onSuccess: () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length === 0) {
      setImageFiles([]);
      setFileError('At least one property image is required.');
      return;
    }
    if (files.length > 10) {
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div className="border-b border-slate-100 pb-5 flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Create Listing</h1>
          <p className="text-sm text-slate-400">Fill in details, locate coordinates, toggle amenities, and upload photos.</p>
        </div>
      ...
      {createMutation.isError && (
        <div className="bg-rose-50 border border-rose-100 text-rose-800 p-4 rounded-xl text-xs font-semibold">
          {createMutation.error?.response?.data?.message || createMutation.error?.message || 'Failed to submit listing. Verify inputs.'}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5 md:col-span-2">
              ...
              <input
                ...
                placeholder="Cozy double sharing room near Manyata Tech Park"
              />
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-4 py-2 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
                placeholder="Give details about house rules, location benefits, rent deposits, and other specs..."
              />
        ...
        {/* Section 2: Address and Coordinates */}
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs space-y-4">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider border-b border-slate-50 pb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5 md:col-span-2">
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20"
                placeholder="Building No 12, 4th Cross"
              />
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] uppercase font-bold text-slate-400">Select Image Files (Max 10)</label>
            <input
              type="file"
              multiple
              accept="image/*"
            ...
            {imageFiles.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {imageFiles.map((file, idx) => (
                  <div key={idx} className="text-[10px] bg-slate-100 border border-slate-200 px-2 py-1 rounded-md text-slate-600 font-semibold">
        ...
        <div className="flex gap-4 items-center justify-end">
          ...
          >
            {createMutation.isPending ? 'Uploading stay...' : 'Submit Listing'}
          </button>
        </div>
